...
impl<'a> LibraryStore<'a> {
    ...
    pub fn find_by_id(&self, id: i64) -> Result<Option<Library>> {
        self.conn.with_connection(|conn| {
            let mut stmt =
                conn.prepare("SELECT id, name, created_at FROM libraries WHERE id = ?1")?;
    ...
    pub fn list(&self) -> Result<Vec<Library>> {
        self.conn.with_connection(|conn| {
            let mut stmt =
                conn.prepare("SELECT id, name, created_at FROM libraries ORDER BY name")?;
