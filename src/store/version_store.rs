...
impl<'a> VersionStore<'a> {
    ...
    pub fn create(&self, version: &NewVersion) -> Result<Version> {
        ...
        let scraper_options = version.scraper_options.as_ref()
        ...
        self.conn.with_transaction(|tx| {
            ...
            Ok(Version {
                ...
                scraper_options: version.scraper_options.as_ref()
...
mod tests {
    use super::*;
    use crate::store::{run_migrations, LibraryStore};
    ...
    fn test_version_crud() {
        ...
        // Create
        let version = store.create(&NewVersion {
        ...
        // Find by ID
        let found = store.find_by_id(version.id).expect("Failed to find version");
        assert!(found.is_some());
        
        // Find by library
        let versions = store.find_by_library(library.id).expect("Failed to find versions");
        assert_eq!(versions.len(), 1);
        
        // Update status
        store.update_status(version.id, VersionStatus::Running).expect("Failed to update status");
        let found = store.find_by_id(version.id).expect("Failed to find version").unwrap();
